...
const DiagnosisSection: React.FC<SectionProps> = ({ form, serverErrors, renderField }) => {
  ...
  const handleRemoveDiagnosisCIE = (id: string) => {
    setDiagnosissCIE(diagnosticosCIE.filter(d => d.id !== id));
  };
  ...
  return (
    <Box sx={{ 
      ...
      <Box sx={{
        ...
        {/* Third Row - Diagnosiss CIE-10 with buttons */}
        <Box sx={{ mb: 2 }}>
          <Box sx={{ 
            display: 'flex',
            alignItems: 'center',
              ...
              <IconButton
                onClick={() => {
                  if (diagnosticosCIE.length > 0) {
                    handleRemoveDiagnosisCIE(diagnosticosCIE[diagnosticosCIE.length - 1].id);
                  }
                }}
                sx={{
            ...
            {diagnosticosCIE.length === 0 ? (
              ...
              diagnosticosCIE.map((diagnostico) => (
                <Chip
                  ...
                  onDelete={() => handleRemoveDiagnosisCIE(diagnostico.id)}
                  size="small"
                  sx={{
