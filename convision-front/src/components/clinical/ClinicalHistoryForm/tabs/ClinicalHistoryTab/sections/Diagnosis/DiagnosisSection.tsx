...
const DiagnosisSection: React.FC<SectionProps> = ({ form, serverErrors }) => {
  // Initialize with three dummy records
  const [diagnosticosCIE, setDiagnosissCIE] = useState<DiagnosisCIE[]>([
    {
      id: 'cie_1',
      ojo: 'OD',
      cie10: 'H52.1',
  ...
  React.useEffect(() => {
    ...
    const currentTipoDiagnosis = form.getValues('tipo_diagnostico');
    if (!currentTipoDiagnosis) {
  ...
  const handleAddDiagnosisCIE = () => {
    // Placeholder for adding CIE-10 diagnostic
    const newDiagnosis: DiagnosisCIE = {
      id: `cie_${Date.now()}`,
      ojo: 'OD',
      cie10: 'H52.0',
    ...
    setDiagnosissCIE([...diagnosticosCIE, newDiagnosis]);
  };

  const handleRemoveSelectedDiagnosiss = () => {
    setDiagnosissCIE(diagnosticosCIE.filter(d => !selectedRows.includes(d.id)));
    setSelectedRows([]);
  };
        ...
        <Box sx={{ 
          ...
          <Box sx={{ flex: 1 }}>
            ...
            <FormControl size="small" fullWidth sx={{ bgcolor: 'white' }}>
              ...
              >
                {tipoDiagnosisOptions.map((option) => (
                  <MenuItem key={option.value} value={option.value}>
                    {option.label}
                  </MenuItem>
                ))}
      ...
      <Box sx={{ mb: 3 }}>
        <Box sx={{ 
          ...
          <Box sx={{ display: 'flex', gap: 1 }}>
            ...
            <IconButton
              onClick={handleRemoveSelectedDiagnosiss}
