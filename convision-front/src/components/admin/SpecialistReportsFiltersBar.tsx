...
const SpecialistReportsFiltersBar: React.FC<Props> = ({
  dateFrom,
  dateTo,
  onRangeChange,
  specialists,
  ...
  const specialistOptions: ComboboxOption[] = [
  ...
  const activeSpecialist =
    selectedSpecialistId !== 'all'
      ? specialists.find((u) => String(u.id) === selectedSpecialistId)
      : null;

  const activeBranch =
    branchFilter !== 'all' ? branches.find((b) => String(b.id) === branchFilter) : null;

  const hasActiveFilters = selectedSpecialistId !== 'all' || branchFilter !== 'all';
  ...
  return (
    <div className="flex flex-col gap-[8px]">
      <div className="flex h-[44px] shrink-0 items-center overflow-hidden rounded-[8px] border border-[#e0e0e4] bg-white">
        ...
        <div className="flex h-full min-w-[160px] flex-col justify-center border-r border-[#f0f0f2] px-[12px]">
          <span className="text-[10px] font-medium tracking-[0.4px] text-[#7d7d87]">ESPECIALISTA</span>
          <SearchableCombobox
            options={specialistOptions}
            value={selectedSpecialistId}
            onChange={onSpecialistChange}
            placeholder={`Todos (${specialists.length})`}
      ...
      {hasActiveFilters && (
        <div className="flex flex-wrap items-center gap-[6px]">
          ...
          {activeSpecialist && (
            <button
              type="button"
              onClick={() => onSpecialistChange('all')}
...
export default SpecialistReportsFiltersBar;
