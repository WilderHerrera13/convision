...
export function useNewSale() {
    ...
    try {
      ...
      if (parsed.selectedLenses?.length > 0) {
        ...
        if (parsed.patientId) {
          ...
          setTimeout(() => {
            newItems.forEach((it) => { if (it.has_discounts) applyBestDiscountAutomatically(it); });
            if (!newItems.some((i) => i.has_discounts)) setTimeout(() => setIsSummaryLoading(false), 500);
          }, 1000);
  ...
  const submitSale = async () => {
    ...
    try {
      ...
      if (!currentPatient) {
        ...
        if (stored) {
          ...
          if (p.patientId && p.patientName) {
            currentPatient = { id: Number(p.patientId), first_name: p.patientName.split(' ')[0] || '', last_name: p.patientName.split(' ').slice(1).join(' ') || '', identification: '', email: '', phone: '' };
      ...
      if (!currentPatient.id || isNaN(patientId) || patientId <= 0) {
        toast({ variant: 'destructive', title: 'Error', description: 'ID de paciente inválido. Por favor regrese e inicie la venta nuevamente.' });
        return;
      }
