...
const BranchDetailPage: React.FC = () => {
  ...
  if (isError || !data) {
    return (
      <PageLayout title="Sede" subtitle="Admin / Sedes">
  ...
  const rows: { label: string; value: string }[] = [
    ...
    { label: 'Estado', value: data.is_active ? 'Activa' : 'Inactiva' },
  ];

  return (
    <PageLayout
      title={data.name}
      subtitle="Admin / Sedes"
      actions={
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            ...
            onClick={() => navigate(`/admin/sedes/${data.id}/edit`)}
          >
