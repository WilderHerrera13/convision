...
class UpdateTreatmentRequest extends FormRequest
{
    ...
    {
        // Assuming only admin or specialist can update treatments
        $user = Auth::user();
    ...
    {
        $treatmentId = $this->route('treatment'); // Assuming route model binding with 'treatment'
        if (is_object($treatmentId)) {
            $treatmentId = $treatmentId->id;
        }

        return [
            'name' => ['sometimes', 'required', 'string', 'max:255', Rule::unique('treatments')->ignore($treatmentId)],
