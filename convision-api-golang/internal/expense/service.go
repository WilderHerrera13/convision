...
func (s *Service) GetStats(db *gorm.DB) (*StatsOutput, error) {
	data, total, err := s.repo.List(db, domain.ExpenseFilter{Pagination: domain.Pagination{Page: 1, PerPage: 10000}})
	if err != nil {
		return nil, err
	}
...
func (s *Service) List(db *gorm.DB, f domain.ExpenseFilter) (*ListOutput, error) {
	f.Clamp()
