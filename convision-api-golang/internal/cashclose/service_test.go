...
func TestSubmit_Success(t *testing.T) {
	...
	repo.On("GetByID", mock.Anything, uint(1)).Return(draftClose, nil).Once()
	repo.On("Update", mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(nil)
	repo.On("GetByID", mock.Anything, uint(1)).Return(&domain.CashRegisterClose{ID: 1, Status: domain.CashRegisterCloseStatusSubmitted}, nil).Once()

	c, err := newCashCloseSvc(repo).Submit(nil, 1, domain.RoleReceptionist, 1)
...
func TestSubmit_NonDraftRejected(t *testing.T) {
	repo := &mocks.MockCashRegisterCloseRepository{}
	repo.On("GetByID", mock.Anything, uint(1)).Return(&domain.CashRegisterClose{ID: 1, UserID: 1, Status: domain.CashRegisterCloseStatusSubmitted}, nil)

	_, err := newCashCloseSvc(repo).Submit(nil, 1, domain.RoleReceptionist, 1)
...
func TestApprove_SubmittedSuccess(t *testing.T) {
	...
	repo.On("GetByID", mock.Anything, uint(1)).Return(&domain.CashRegisterClose{ID: 1, Status: domain.CashRegisterCloseStatusSubmitted}, nil).Once()
	repo.On("Update", mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(nil)
	repo.On("GetByID", mock.Anything, uint(1)).Return(&domain.CashRegisterClose{ID: 1, Status: domain.CashRegisterCloseStatusApproved}, nil).Once()

	c, err := newCashCloseSvc(repo).Approve(nil, 1, 2, cashclose.ApproveInput{})
...
func TestApprove_NonSubmittedRejected(t *testing.T) {
	repo := &mocks.MockCashRegisterCloseRepository{}
	repo.On("GetByID", mock.Anything, uint(1)).Return(&domain.CashRegisterClose{ID: 1, Status: domain.CashRegisterCloseStatusDraft}, nil)

	_, err := newCashCloseSvc(repo).Approve(nil, 1, 2, cashclose.ApproveInput{})
...
func TestGetByID_WrongUserForbidden(t *testing.T) {
	repo := &mocks.MockCashRegisterCloseRepository{}
	repo.On("GetByID", mock.Anything, uint(1)).Return(&domain.CashRegisterClose{ID: 1, UserID: 1, Status: domain.CashRegisterCloseStatusDraft}, nil)

	_, err := newCashCloseSvc(repo).GetByID(nil, 1, domain.RoleReceptionist, 2)
